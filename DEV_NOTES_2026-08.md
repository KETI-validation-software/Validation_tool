...
1. **403 유도 방법** — 무엇을 어떻게 보내야 403인가? (토큰 변조라면 401 인증
   오류/402 토큰 만료와 어떻게 구분해 유도하는지)
2. **오류를 걸 "특정 메시지" 선정 방식** — 관리도구에 미리 등록(현행처럼 규칙의
   code 기대값으로 표현)인지, 매 회차 무작위 선택인지 (무작위면 도구 개발 추가)
3. **오류 회차 점수 처리** — 정상 점수와 합산인지 별도 표기인지, 오류 회차
   실패 시 해당 API 전체 FAIL인지
4. **관리시스템 일정** — 시험범위 enum 4종(필수/전체/필수-오류/전체-오류) 확장
