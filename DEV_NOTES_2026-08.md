...
| 3 | 장치 역할 오류 응답 로직 **부활** (`api_server.py` 주석 해제) + String 시각 대응 + 판별 기준을 유도 방식과 짝 맞춤(startTime "0"→201, 없는 ID→404, 타입 오류→400) + 403 분기 신설 | 중 | 박사님 승인 (§2 참고) |
...
5. **api_server.py 오류 응답 로직(400/201/404) 주석 처리** — 의도였는지, 이번에
   부활시켜도 되는지
