...
## 8. 오류 처리 시험 — 확정된 그림 (2026-08-16 박사님 문서 + 08-17 논의)

박사님 문서("물리보안 시스템 상호운용성 시험 — 오류 처리(⑩) 케이스 정리")로
이전의 추측이 대부분 정리됐다. 확정 사항:

- **시험범위 enum은 필수/전체 2종 그대로.** "필수-오류" 같은 항목이 새로 생기지
  않는다. 오류 처리는 별도 신청 항목이 아니라 신청한 기능의 모든 시나리오에
  공통 적용되는 단계다. → 관리시스템 enum 확장(엘컴택 패치) 불필요.
- **오류 코드는 201·400·403·404 넷뿐.** 그 외(401·402·405 등)는 이번 시험에서
  쓰지 않는다. 특히 기존 api_server의 401 응답들은 **403으로 교체 대상**(잔재).
- **적용 시험: 단일시스템 시험만.** 통합시스템 시험에는 오류 시나리오가 없다.
  피시험 통합시스템의 잘못된 요청은 기존 요청 필드 검증이 채점한다.
- **대상 메시지는 사전 지정** (박사님 회신). 문서의 "무작위"는 실행 시점 추첨이
  아니라 "전 메시지가 아닌 일부에만 주입"이라는 뜻. 무작위 선택 로직 불필요.
  단, "어느 메시지에 어떤 방법"까지의 정의는 관리도구/절차서에서 확정 필요
  (문서의 표 3·5·7이 사실상 그 초안).
- **정상 수행 → 오류 주입** 2단 구성(문서 명시). 2단 실행 흐름은 미구현.

### 주입 방법 7종 × 기대 코드 (문서 표 1)

| # | 주입 방법 | 기대 | 필수 범위 | 전체 범위 |
|---|---|---|---|---|
| ① | 저장 조회 구간 밖 timePeriod | 201 | 적용 | 적용 |
| ② | 필수 필드 누락 | 400 | 적용 | 적용 |
| ③ | 필드 자료형 불일치 | 400 | 필수 필드 대상만 | +선택 필드 |
| ④ | 유효 값 위반 | 400 | 필수 필드 대상만 | +선택 필드 |
| ⑤ | 액세스 토큰 미포함 | 403 | 토큰 인증 구현 시 | 적용 |
| ⑥ | 접근 불가 URL 호출 | 403 | 적용 | 적용 (**경로 기준 절차서 확정 대기**) |
| ⑦ | 미등록 장치 ID | 404 | 적용 | 적용 |

**범위의 의미**: 필수 범위 시험은 업체가 선택 필드를 구현 안 했을 수 있으므로
**필수 필드만 주입 표적**으로 삼는다. 범위가 바꾸는 것은 커버 가능한 코드
종류가 아니라 **케이스 수**다(4개 코드 모두 필수 필드만으로 성립. 201만
범위가 아니라 "저장 기능 신청 여부"에 묶임). 시각 필드 Number 예시는 우리
String 전환과 어긋나지만 박사님 확인 결과 **예시 오류로 무시**.

### 검증 계위 (2026-08-17 논의로 정리)

오류 검증은 제3의 검증이 아니라 **맥락 검증의 code 규칙(관문)에 기대값만 오류
코드로 넣은 것**이다. 흐름은 정상/오류 회차가 동일하다:

```
응답 도착 → [관문] code == 기대값?
  불일치 → 조기 종료, 전체 필드 실패 (functions.py:366)
  일치   → 규격·맥락 검증으로 필드별 채점
           (오류 회차는 남는 필드가 code·message 둘뿐 → 관문 통과 = 사실상 끝)
```

정상 회차에서는 validation_registry가 오류 기대 규칙을 걷어내 충돌을 막는다.
오류 회차의 점수는 관문 실패 시 해당 API 전체 필드 0점(기존 구현 그대로).

### 역할 정리 (2026-08-17 확정 — 혼동 주의)

| 시험 | 우리 도구 | 오류 기능 |
|---|---|---|
| 단일시스템 시험 | systemVal_all (요청 송신) | **주입 + 채점** — 실제 시험용 제품 기능 |
| 통합시스템 시험 | platformVal_all + api_server (요청 수신) | 없음 — 잘못된 요청은 기존 필드 검증이 채점 |
| 리허설 | 둘을 마주 세움 | api_server의 오류 판정 = **리허설 상대역** (피시험 단일시스템 흉내) |

api_server의 오류 판정(_check_request_errors)은 실제 시험 채점에 안 들어간다.
단, 인증 실패 시 401→403 교체는 실제 통합 시험에서도 피시험 시스템이 보는
동작이므로 리허설용으로 깎으면 안 됨.

### 도구 개발 항목

| # | 항목 | 규모 | 상태 |
|---|---|---|---|
| 1 | 인증 메시지 주입 제외 | 소 | ✅ 완료 (a1b5008) |
| 2 | ~~무작위 선택 로직~~ | — | ❌ 불필요 (사전 지정 방식 확정) |
| 3 | 주입 ②필수누락·④유효값위반·⑦미등록ID 추가 + ③범위 제한 + ⑤토큰 미포함 | 중 | ✅ 완료 (8b210a2) |
| 4 | 주입 방법을 지정받는 통로 (메시지·필드·방법) — `_applied_codevalue(method=...)` 인자는 준비됨 | 중 | 관리도구/절차서 협의 대기 |
| 5 | 정상 회차 → 오류 회차 2단 실행 흐름 | **대** | 스펙 표현 확정 대기 |
| 6 | 리허설 상대역 판정 확장 (②④⑤⑦ 판정 + 401→403 교체) | 중 | ⏳ 진행 중 — 아래 "작업 7" 참고 |
| 7 | ⑥ 접근 불가 URL (주입 + 상대역 판정) | 중 | 절차서 경로 기준 확정 대기 |
| 8 | `ENABLE_ERROR_REQUEST_MUTATION` 수동 플래그를 회차 판별로 대체 | 소 | 5번 이후 |
...
### 해소된 질문들 (기록용) / 남은 확인 사항

문서와 박사님 회신·08-17 논의로 해소됨:
- ~~계위 확정~~ → 시험범위 enum 2종 유지, 오류는 공통 적용 단계 (문서)
- ~~403 유도 방법~~ → ⑤토큰 미포함 + ⑥접근 불가 URL (문서 표 1)
- ~~대상 메시지 선정~~ → 사전 지정. "무작위"는 일부에만 주입한다는 뜻 (회신)
- ~~시각 필드 Number 예시~~ → 예시 오류, String 전환이 기준 (회신)
- ~~인증 실패 응답 코드~~ → 4코드 체계이므로 401 잔재는 403으로 교체 (08-17 결정)

남은 확인:
1. **⑥ 접근 불가 URL — 설계 합의됨(2026-08-18), 구현은 미착수**
   - 주입: 본문 그대로, 경로만 다른 분야 것으로 치환. 고정 표 —
     출입통제·센서 시험 → `/video/StreamURLs`, 영상 시험 → `/ac/DoorProfiles`
   - 판정(상대역): 시나리오 목록에 없는 API 요청은 **전부 403 권한 없음**.
     4코드 체계에 "API 없음" 코드가 없고, 표준 403 정의가 "접근 불가 URL 호출"
     이므로 남의 분야든 미지의 이름이든 구분 불필요. 404는 장치 ID 미등록 전용.
     (기존 `404 "API를 찾을 수 없습니다"` 응답 교체. 단 `404 "호출 횟수 초과"`는
     리허설 진행 제어용 내부 장치라 유지 — 정의와 안 맞는 잔재로만 기록)
   - 403 유도 방법 자동 선택: 인증이 Bearer면 ⑤ 토큰 미포함, Digest면 ⑥ 경로
     치환 (Digest는 "뺄 토큰"이 없어 ⑤ 불성립 — 인증 생략은 오류가 아니라
     challenge 절차의 정상 첫걸음. 문서 ※1과 일치)
2. **"어느 메시지에 어떤 방법" 정의의 전달 방식** — 문서 표 3·5·7을 확정본으로
   봐도 되는지, 관리도구로 내려줄지 도구 내 설정으로 둘지
3. **오류 회차 점수의 표기** — 정상 회차와 합산인지 별도 표기인지
   (전패 처리 로직 자체는 구현돼 있음 — functions.py:366)
4. **"정상 후 오류" 2단의 실체 — 해석 A/B 확인 (2026-08-18 논의, 박사님 확인 대기)**
   - 해석 A: 한 시험 안에서 도구가 정상 1바퀴 → 주입 2바퀴를 연속 실행.
     → 회차 루프·표시·점수 합산·스펙 표현 전부 신규 개발 (대형)
   - 해석 B: 정상 버전/오류 버전 **시나리오가 관리도구에 따로 등록**돼 있고,
     정상만 신청해도 오류 버전이 default로 이어서 돌아감.
     → **도구는 현 구조 그대로** (오류 버전 스펙 = code 기대값이 오류 코드인
     스펙, 이미 동작). 남는 일은 관리도구 등록(표 3·5·7 이관)과 결과 표기 확인.
     이 구조에선 ENABLE_ERROR_REQUEST_MUTATION을 항상 켜둬도 됨 — 정상 스펙은
     기대값 200이라 주입 분기를 안 탄다.
   - 박사님 회신("사전에 어느 메시지에 어떤 값을 보낼지 정의는 해야")과
     "별도 신청 항목이 아니다"는 문구는 해석 B와 부합. **B로 확정되면
     "2단 실행 흐름(대형)" 항목은 "관리도구 등록 대기"로 강등.**

### ENABLE_ERROR_RESPONSE_CHECK 운영 규칙 (2026-08-18 논의)

리허설·본시험 모두 **True 유지가 기본값**. 잘못된 요청에 오류 코드로 답하는 것이
표준 부합이고, 시험은 절차서대로 감독 하에 진행되므로 피시험 시스템이 오류
응답을 받았다고 중단하지 않는다(각본 진행). 채점(요청 필드 검증)은 응답 경로와
독립이라 어느 쪽이든 동일하게 기록된다.

- 이력: 2026-08-16 이전의 모든 시험은 이 로직이 주석 처리돼 **항상 200+데이터로
  진행**하는 방식이었다. True 운영은 방식 변경이므로 박사님께 한 줄 공유할 것.
- 남는 효과 하나: 피시험 시스템이 잘못된 요청으로 400을 받으면 그 응답에 실렸을
  데이터(장치 목록 등)를 못 받은 채 다음 단계를 진행한다 — 자기 오류의 자연스러운
  귀결로 본다.
- 오탐 등 문제 시 외부 CONSTANTS.py에서 False → 즉시 이전 동작(항상 200) 복귀.

### 오류 회차 채점 규칙 (e000c1f, 2026-08-18 실측 후 수정)

- 기대 오류 코드가 **그대로 오면**: code·message 두 필드만 채점 (오류 응답의
  본문은 그 둘뿐인 것이 정상 — 정상 응답용 스키마로 재면 "필드 누락" 오탐)
- 기대 오류 코드와 **다른 코드가 오면**: 기존대로 전체 필드 0점 조기 종료
- 축소 발동은 code 규칙으로만 판단 (message 문구는 "성공" 등 정상 문구와 겹쳐
  오발동 위험)
- 실측: ac002 StoredVerifEventInfos 201 유도 성공이 화면에서 FAIL로 뜨던 문제의
  원인이자 수정. 회귀: temp/test_error_round_scoring.py
