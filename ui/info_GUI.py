...
class InfoWidget(QWidget):
    ...
    def initUI(self):
        ...
        # 버전3: 자동 로드 플래그 (showEvent에서 한 번만 실행)
        self._auto_load_done = False

    def showEvent(self, event):
        """창이 표시될 때 호출 (버전3: 자동 로드)"""
        super().showEvent(event)

        # 버전3: 창이 완전히 표시된 후 자동으로 시험 정보 불러오기 (한 번만 실행)
    ...
    def _auto_load_test_info(self):
        """버전3: config.txt의 test_ip로 자동으로 시험 정보 불러오기"""
        try:
            ip_address = CONSTANTS.test_ip
            if not ip_address:
                Logger.debug("자동 로드 실패: test_ip가 설정되지 않음")
                return
            ...
            self.loading_popup.update_message("시험 정보 불러오는 중...", f"IP: {ip_address}")
    ...
    def _determine_mode_from_api(self, test_data):
        ...
        try:
            ...
            Logger.debug(f"verificationType 추출 실패: {e}")
            import traceback
            traceback.print_exc()
            return "request"  # 기본값
        ...
        try:
            ...
            if self.test_port:
                ...
                try:
                    ...
                    Logger.debug(f"CONSTANTS.py 파일 수정 실패: {e}")
    ...
    def on_management_url_changed(self):
        ...
        try:
            ...
            Logger.debug(f"관리자시스템 주소 변경 처리 실패: {e}")
    ...
    def check_next_button_state(self):
        ...
        try:
            ...
            Logger.debug(f"다음 버튼 상태 체크 실패: {e}")
    ...
    def _is_page1_complete(self):
        ...
        try:
            ...
            Logger.debug(f"페이지 완료 조건 체크 실패: {e}")
            return False
